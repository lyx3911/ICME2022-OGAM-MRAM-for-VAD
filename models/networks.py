...
def define_G(input_nc, output_nc, ngf, netG, norm='batch', use_dropout=False, init_type='normal', init_gain=0.02,
    ...
    elif netG == 'resnet_6blocks_attention_pro':
        net = ResnetAttentionGenerator_Pro(input_nc, output_nc, ngf, norm_layer=norm_layer, use_dropout=use_dropout, n_blocks=6)
    elif netG == 'unet_128':
...
class ResnetAttentionGenerator(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=6,
        ...
        # generate attention 
        r = 8
        self.attention_meta_learner = nn.Sequential(nn.Linear(4096, int(4096/8)), nn.ReLU(),
                                                    nn.Linear(int(4096/r), 4096) ) # 这里代表整个网络的frame输入尺寸为256*256, 所以attention处理的feature为64*64
...
class ResnetAttentionGenerator_Pro(ResnetAttentionGenerator):
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=6,
                 padding_type='reflect', step=4):
        ResnetAttentionGenerator.__init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=6,
                 padding_type='reflect', step=4) 
    
    def forward(self, frame_objects_batch):
        """
        frame_objects_batch.type: list
        frame_objects_batch.len: T = 4
        frame_objects[i][0] is frame i
        frame_objects[i][1]  is object_stack in frame i
        """
        T = len(frame_objects_batch)
        z = []
        for i in range(T):
            frame = frame_objects_batch[i][0]
            frame = frame.reshape(-1, frame.shape[-3], frame.shape[-2], frame.shape[-1])
            objects = frame_objects_batch[i][1]
            objects = objects.reshape(-1, objects.shape[-3], objects.shape[-2], objects.shape[-1])

            # print(frame.shape, objects.shape)
            ft = self.model(frame)
            b = self.model(objects)
            b_view = b.view(b.shape[0], b.shape[1], -1)
            attention_map = self.get_attention(ft.view(ft.shape[1], -1),b_view)
            zt = attention_map * ft
            z.append(zt)
...
class ResnetGenerator(nn.Module):
    ...
    def forward(self, input):    # input: N x T x c x h x w
        ...
        z = z.reshape(-1, T, z.shape[-3], z.shape[-2], z.shape[-1])
        out_recurrent, _ = self.recurrent_model(z) # z: N x T x (ngf * 2 ** n_downsampling) x h' x w'
        out_recurrent = out_recurrent[0].reshape(out_recurrent[0].shape[0], -1, out_recurrent[0].shape[-2], out_recurrent[0].shape[-1])
        out = self.de_model(out_recurrent)
